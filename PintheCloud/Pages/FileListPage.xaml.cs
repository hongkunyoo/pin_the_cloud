...
namespace PintheCloud.Pages
{
    public partial class FileListPage : PtcPage
    {
        ...
        private async void Refresh()
        {
            ...
            List<FileObject> fileList = await App.BlobStorageManager.GetFilesFromFolderAsync(this.AccountId, this.SpaceId);
            if (fileList.Count > 0)
            {
        ...
        private async Task UploadFileAsync(FileObjectViewItem file)
        {
            ...
            if (stream != null)
            {
                uploadPath = await App.BlobStorageManager.UploadFileThroughStreamAsync(this.AccountId, this.SpaceId, file.Name, stream);
                if (uploadPath == null)
                {
